...
// Fetch top guides with limit
export const fetchTopGuides = async (limit = 8) => {
  try {
    const response = await fetch(
      `${BASE_URL}/guider/all-guider?page=1&limit=${limit}`
    );

    if (!response.ok) {
      throw new Error('Failed to fetch top guides');
    }

    const result = await response.json();

    if (result.success) {
      // Transform and sort guides by rating
      const formattedGuides = result.data
        .map(guide => ({
          id: guide._id || guide.id,
          name: guide.name,
          location: guide.location || "Unknown",
          ...
          reviews: Math.floor(Math.random() * 100),
          experience: guide.experience || "5+ years",
          imageUrl: guide.profile_image || "/images/home/guide.jpg",
          specialty: guide.specialty || ["Cultural Tours"],
          verified: guide.verified || true
        }))
      ...
      throw new Error(result.message || 'Failed to fetch top guides');
    }
  } catch (err) {
