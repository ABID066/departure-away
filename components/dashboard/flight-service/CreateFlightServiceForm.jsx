...
export default function CreateFlightServiceForm() {
    ...
    useEffect(() => {
        try {
            ...
            if (userData) {
                ...
                // Check if user role is allowed to create flight services
                if (user.role === "user") {
                    setUnauthorized(true);
                    setError("Only freelancers and agencies can create flight services.");
                    return;
                }
    ...
    const handleSubmit = async (e) => {
        ...
        if (unauthorized) {
            setError("You are not authorized to create flight services.");
            return;
        }
        ...
        try {
            ...
            const response = await fetch('https://royolex.vercel.app/api/v1/flight/create-flight', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                throw new Error(result.message || 'Failed to create flight service');
            }

            console.log("Flight service created successfully:", result);
            // Redirect to flight service listing
            router.push('/dashboard/flight-service');

        } catch (error) {
            console.error("Error creating flight service:", error);
            setError(error.message || "Failed to create flight service. Please try again.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="p-4 md:p-6">
            <div className="bg-white rounded-lg shadow p-4 md:p-6">
                <h2 className="text-lg md:text-xl font-semibold mb-4 md:mb-6">Create New Flight Service</h2>
                ...
                {unauthorized ? (
                    <div className="text-center py-8">
                        <div className="mb-4 text-red-500 font-medium">
                            You don't have permission to create flight services.
                        </div>
                        <p className="mb-4 text-gray-600">
                            Only freelancers and agencies can create flight services. If you believe this is an error, please contact support.
                        </p>
                        <button
                            onClick={() => router.push('/')}
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="mt-6 md:mt-8 flex flex-col sm:flex-row sm:justify-end gap-3">
                            <button
                                ...
                                onClick={() => router.push('/dashboard/flight-service')}
                            ...
                            >
                                ...
                                ) : isUploading ? 'Uploading Images...' : 'Create Flight Service'}
                            </button>
                        </div>
                    </form>
                )}
