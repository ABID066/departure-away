...
export default function Navbar() {
  ...
  return (
    <nav className="relative max-w-7xl mx-auto text-center">
            ...
            <div className="md:hidden mt-4 space-y-4">
              ...
              <div className="py-2">
                {languages.map((language) => (
                  <button
                    key={language.name}
                    className={`block w-full text-left py-2 text-black hover:text-purple-200 transition-colors ${
                      selectedLanguage === language.name ? "text-pink-700" : ""
                    }`}
                    onClick={() => {
                      setSelectedLanguage(language.name);
                      setIsOpen(false);
              ...
              <button
                className="text-black border border-black px-4 py-2 rounded-full hover:bg-pink-700 hover:border-transparent hover:text-amber-50 transition-colors"
                onClick={() => setIsOpen(false)}
              >
                Get Started
              </button>
